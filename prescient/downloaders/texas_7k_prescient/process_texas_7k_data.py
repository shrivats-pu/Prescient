...
def gmlc_to_prescient_by_zone(source, zone_numbers = [1,2,3], forecast_error=False):
    """
    This takes the RTS-GMLC time series data and 
    puts it into the format required for prescient, 
    aggregated by zone (1, 2, or 3 for RTS-GMLC)
    :param source: options are WIND, PV, RTPV, or Hydro
    :param forecast_error: says whether forecast is in 5 min increments or not
    :return: writes csv files of forecast/actual in prescient format
    """
    # Find the files you want, forecast and actual:
    data_folder = os.path.join(path,source)
    ...
    if forecast_error==True:
        """
        actual_raw = pd.read_csv(actual_file)
        dt_5min = pd.date_range(dt[0], periods=actual_raw.shape[0], freq='H')
        actual = actual_raw[site_list]
        actual = actual.assign(time5min = dt_5min.values)
        actual = actual.set_index('time5min')
        actual = actual.resample('H').mean()
...
def load_by_bus(source, zone_numbers = [1,2,3], forecast_error=True):
    """
...
def source_contribution_factors(source, zone_numbers, by_zone=True):
    """
...
def create_timeseries(base_dir=None, path_prefix = "TEXAS-7K/TX_Data", forecast_error = True, zones=tx_zones):
    ...
    path = os.path.normpath(os.path.join(base_dir, path_prefix +"/timeseries_data_files/"))
    source_path = os.path.normpath(os.path.join(base_dir, path_prefix + "/SourceData/"))
    ...
    gmlc_to_prescient_by_zone('WIND', forecast_error=forecast_error, zone_numbers=zones)
    gmlc_to_prescient_by_zone('PV', forecast_error=forecast_error, zone_numbers=zones)
    gmlc_to_prescient_by_zone('Load', forecast_error=forecast_error, zone_numbers=zones)
    ...
    load_by_bus('Load', forecast_error=forecast_error, zone_numbers=zones)

    source_contribution_factors('WIND', by_zone=True, zone_numbers=zones)
    source_contribution_factors('WIND', by_zone=False, zone_numbers=zones)
    source_contribution_factors('PV', by_zone=True, zone_numbers=zones)
    source_contribution_factors('PV', by_zone=False, zone_numbers=zones)
